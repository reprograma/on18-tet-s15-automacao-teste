...
const atualizarCozinha = async(req, res) => {    
    const { nome, cnpj, telefone, iniciativa_privada,
    endereco, bairros_atuantes, site, atividades_disponiveis,
    pessoa_responsavel } = req.body
    ...
    try {
        ...
        if(!procurarCozinha) return res.status(404).json({
            message: `Cozinha ${id} not found`
        })
        ...
        const cozinhaAtualizada = await procurarCozinha.save()
